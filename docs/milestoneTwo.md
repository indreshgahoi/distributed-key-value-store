...
- `BenchmarkRaft_ConcurrentProposals` — proposal throughput under concurrent write pressure. **Must be run with a fixed `-benchtime` (e.g. `-benchtime=600x`)** — left to Go's default adaptive scaling, it proposes enough unique keys to fill the test cluster's fixed-size Layer 0 arena; the storage error then halts the nodes (fail-stop), failing the benchmark (see [bench/README.md](../bench/README.md)). This cap dropped from 2000x to 600x once persistence landed, since every `AppendEntries` now also writes into the raft node's own storage arena on top of the MVCC arena.
