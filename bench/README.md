...
- `BenchmarkRaft_ConcurrentProposals` proposes unique keys as fast as `b.RunParallel` allows. Left to Go's default adaptive `-benchtime` (scale `b.N` until ~1s), it reliably exhausts the 16MB test-cluster arena and **panics the whole test binary** (`raw: arena out of memory` — Layer 0's bump allocator has no reclamation and errors via `panic`, not a returned error). Always pin the iteration count explicitly:
  ...
  ```
  This cap dropped from 2000x to 600x once Raft's `KVStorage` started persisting log entries: every `AppendEntries` now also JSON-encodes and writes the entry into the node's own 16MB raft-storage arena, on top of the existing MVCC arena — so the same iteration count now consumes roughly twice the arena budget it used to. (That storage write doesn't yet survive a process restart — `KVStorage` currently sits on Layer 0's in-memory `SkipListEngine`, not a disk-backed one — but it consumes the same in-memory arena budget either way. See [docs/milestoneTwo.md §7](../docs/milestoneTwo.md#7-storage-durability--log-persistence).) Re-check this number if the write path's per-entry footprint changes again.
